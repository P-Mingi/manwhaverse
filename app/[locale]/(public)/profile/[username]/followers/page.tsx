...
export async function generateMetadata({ params }: FollowersPageProps) {
  ...
  return { title: `${username} — ${t('followersList')}` }
}
...
export default async function FollowersPage({ params, searchParams }: FollowersPageProps) {
  ...
  const { users, total } = await getFollowers(profileUser.id, currentPage)
    ...
    <PageContainer>
      <div className="mb-6">
        ...
        <h1 className="mt-2 font-display text-xl font-bold">
          {t('followersList')} ({total})
        </h1>
      </div>

      {users.length === 0 ? (
        <p className="text-sm text-text-muted">{t('noFollowers')}</p>
      ) : (
      ...
      {totalPages > 1 && (() => {
        ...
        <div className="mt-6 flex justify-center gap-2">
          {_pages.map((p) => (
              <a
                key={p}
                href={`/${locale}/profile/${username}/followers?page=${p}`}
