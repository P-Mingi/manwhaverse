...
export function MobileNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-electric-border bg-deep/90 backdrop-blur-md md:hidden">
          ...
          return (
            <Link
              ...
              className={`relative flex flex-col items-center gap-0.5 px-3 py-1 text-xs transition-colors ${
                isActive ? 'text-electric' : 'text-text-muted'
              }`}
            >
              <div className="relative">
                <item.icon active={isActive} />
                {item.badge != null && item.badge > 0 && (
                  <span className="absolute -right-1.5 -top-1 flex h-3.5 min-w-3.5 items-center justify-center rounded-full bg-red px-0.5 text-[8px] font-bold text-white">
