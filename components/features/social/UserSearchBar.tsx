...
export function UserSearchBar({ locale, isLoggedIn }: Props) {
  const lang = locale === 'fr' ? t.fr : t.en
  ...
  const search = useCallback(async (q: string) => {
    ...
    try {
      ...
      // Sync follow states from fresh data
      ...
      {open && (
        <div className="absolute left-0 right-0 top-full z-50 mt-1.5 overflow-hidden rounded-xl border border-border bg-surface shadow-lg">
          {results.length === 0 ? (
            ...
            <ul>
              {results.map((user) => {
                const isFollowing = followStates[user.id] ?? user.is_following
                return (
                  <li
                    key={user.id}
                    ...
                    {isLoggedIn && (
                      <button
                        ...
                        className={`flex-shrink-0 rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${
                          isFollowing
                            ? 'bg-elevated text-text-secondary hover:bg-border hover:text-text-primary'
                            : 'bg-accent text-white hover:bg-accent/80'
                        }`}
                      >
                        {isFollowing ? lang.unfollow : lang.follow}
                      </button>
                    )}
                  </li>
                )
