...
import { toggleLikeAction, toggleDislikeAction, toggleReactionAction } from '@/lib/actions/review'
...
export function ReviewCard({ review, currentUserId, locale, manhwaSlug, preview = false }: ReviewCardProps) {
  ...
  return (
    <div className="rounded-lg border border-border bg-surface p-4">
      ...
      {shouldTruncate && reviewHref && (
        <Link
          href={reviewHref}
          className="mt-1 inline-block text-xs text-crystal-blue hover:underline"
        >
          {t('readFull')}
        </Link>
      )}
      ...
      <div className="mt-2 flex items-center gap-4">
        <button
          onClick={handleLike}
          disabled={isPending || !currentUserId}
          className="flex items-center gap-1 text-xs text-text-muted transition-colors hover:text-crystal-blue disabled:opacity-50"
        >
        ...
        {isLong && reviewHref && !shouldTruncate && (
          <Link
            href={reviewHref}
            className="ml-auto text-xs text-text-muted transition-colors hover:text-crystal-blue"
          >
            {t('readFull')}
          </Link>
        )}
